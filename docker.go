...
const (
    ...
    maxRetries  = 30    // 🔹 Maximum attempts to check JupyterLab readiness
    checkDelay  = 2     // 🔹 Delay (seconds) between retries
)
...
// waitForJupyterLab checks if JupyterLab is responding before returning the container info
func waitForJupyterLab(port int) {
    ...
    for i := 0; i < maxRetries; i++ {
        ...
        if err == nil {
            ...
            if resp.StatusCode == http.StatusOK || resp.StatusCode == http.StatusFound {
                log.Printf("✅ JupyterLab is ready on %s", url)
                return
            }
        }
        log.Printf("⌛ Waiting for JupyterLab to start... (%d/%d)", i+1, maxRetries)
        time.Sleep(time.Duration(checkDelay) * time.Second)
    }

    log.Fatalf("❌ JupyterLab did not start within the timeout period")
}
...
func getOrSpawnContainer(userID string) *ContainerInfo {
    ...
    if len(containers) > 0 {
        ...
        // 🔹 Wait for JupyterLab to be ready before returning the container info
        waitForJupyterLab(existingPort)
